class textFrame:
    ...
    '''
    def __init__(self, own_template = None, align = 'left', screen_width=80, fill_with=' '):
        self.version = '1.01'
        self.ordinary_template = '┌─┐└─┘││'
        self.double_template  =  '╔═╗╚═╝║║'
        self.light_shadow_template = '┌─╖╘═╝│║'
        self.shadow_template='┌─ └▄█│█'
        self.align = align.lower() if align in ['right','rjust','left','ljust','center','r','l','c'] else 'left'
        self.width = screen_width if int(screen_width) in range(1, 255) else 80
        self.fill = fill_with if len(fill_with)==1 else fill_with[0]
        
    def ordinary(self, text):
        self.print_me(text, self.ordinary_template)

    def doubled(self, text):
        self.print_me(text, self.double_template)        

    def light_shadow(self, text):
        self.print_me(text, self.light_shadow_template)   

    def shadow(self, text):
        self.print_me(text, self.shadow_template)     
        
    def print_me(self,text,template):
        t = template
        l = len(text)
        up = t[0] + t[1]*l + t[2]
        ct = t[-2]+text+t[-1]
